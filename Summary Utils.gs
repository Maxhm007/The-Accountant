...
function refreshSummary() {
  ...
  function ema_(arr, len) {
    ...
    for (let i = 0; i < arr.length; i++) {
      const v = Number.isFinite(arr[i]) ? arr[i] : e;
      e = (i ? (v - e) * k + e : v);
      out[i] = e;
    }
    return out;
  ...
  function macdHist_(cl) {
    ...
    const e12 = ema_(cl, 12);
    const e26 = ema_(cl, 26);
    const macd = cl.map((_, i) => e12[i] - e26[i]);
    const sig = ema_(macd, 9);
  ...
  function trendPillars_(series, buyStartMs) {
    ...
    const N = 10;
    if (hist.length < Math.max(N, 20)) return { sym: '=', score: 0, flags: { insufficient: true } };
    const lastN = hist.slice(-N);
    const clAll = hist.map(x => x.close).filter(Number.isFinite);

    // Pillar 1: Location
    const e20 = ema_(clAll, 20);
    const lastClose = clAll[clAll.length - 1];
  ...
  function readDSEX_CloseSeries_(ss, cutoffMs, buyStartMs) {
    const sh = ss.getSheetByName(DSEX_SHEET);
    if (!sh) return null;
    ...
    const iDSEX = H.findIndex(h => h.includes('DSEX INDEX'));
    if (iDate < 0 || iDSEX < 0) return null;

    const vals = sh.getRange(2, 1, lr - 1, lc).getValues();
    const ser = [];
    for (const r of vals) {
      const d = r[iDate] instanceof Date ? r[iDate] : new Date(r[iDate]);
      if (isNaN(d)) continue;
      const ms = d.getTime();
      if (ms < cutoffMs || ms >= buyStartMs) continue;
      const c = Number(r[iDSEX]);
      if (Number.isFinite(c)) ser.push({ ms, close: c });
    }
    ser.sort((a, b) => a.ms - b.ms);
  ...
  function regimeFromCloses_(cl) {
    ...
    if (strongUp) return { ok: true, sym: '⬆️' };
    if (neutralOk) return { ok: true, sym: '=' };
  ...
  const ltp = ss.getSheetByName(LTP_SHEET);
  const hdata = ss.getSheetByName(HDATA_SHEET);
  if (!ltp || !hdata) return safeWrite_([], ACTION_LIST, 'Missing LTP/HData');

  const lr = ltp.getLastRow();
  const lc = ltp.getLastColumn();
  ...
  const findHeader = (keys) => {
    ...
    for (let i = 0; i < NU.length; i++) {
      for (const k of arr) {
        if (NU[i].includes(norm(k))) return i;
      }
    }
    return null;
  };
  ...
  const ltpVals = ltp.getRange(2, 1, lr - 1, lc).getValues();
  ...
  const ltpPriceByCode = new Map();
  for (const r of ltpVals) {
    const code = norm(r[colCode]);
    if (!code) continue;
    if (colLtpPx != null) {
      const px = toNum(r[colLtpPx]);
      if (Number.isFinite(px)) ltpPriceByCode.set(code, px);
    }
  }
  ...
  const h_lr = hdata.getLastRow();
  if (h_lr < 2) return safeWrite_([], ACTION_LIST, 'HData empty');
  ...
  const hVals = hdata.getRange(2, 1, h_lr - 1, h_lc).getValues();
  DBG.hRows = hVals.length;

  const seriesByCode = Object.create(null);
  const todayHMap = new Map();
  let hasHDataToday = false;

  for (let r = 0; r < hVals.length; r++) {
    const row = hVals[r];
    const code = norm(row[iCode]);
    if (!code || !LTP_CODES.has(code)) continue;
    const d = row[iDate] instanceof Date ? row[iDate] : new Date(row[iDate]);
    if (isNaN(d)) continue;
    const ms = d.getTime();
    if (ms < cutoffMs) continue;
    if (!AFTER_1430 && ms >= todayStartMs) continue;
    const item = {
      ms,
  ...
  function medianOfLastN_(arr, n) {
    const a = arr.slice(-n).filter(Number.isFinite);
  ...
  for (const [code, arr] of Object.entries(seriesByCode)) {
    ...
    const prevC = prevClose.get(code);
    if (!Number.isFinite(prevC) || prevC <= 0) continue;
    const cl = arr.map(x => x.close);
    const e20 = ema_(cl, 20);
  ...
  const existing = (function () {
    ...
    if (!sh || sh.getLastRow() < 2) return { map, orderPurchased, orderWatch };
  ...
  function lastNBefore_(series, buyStartMs2, n, picker) {
    const vals = [];
    for (let k = series.length - 1; k >= 0 && vals.length < n; k--) {
      const row = series[k];
  ...
  function valueInc_Last1vs3Avg_(series, buyStartMs2, d0ValMn) {
    ...
    const prev3 = lastNBefore_(series, buyStartMs2, 3, r => r.val);
    if (prev3.length < 3) return { txt: '', num: NaN };
    const avgPrev3 = (prev3[0] + prev3[1] + prev3[2]) / 3;
    if (!Number.isFinite(avgPrev3) || avgPrev3 <= 0) return { txt: '', num: NaN };
    const changePct = (d0ValMn / avgPrev3 - 1) * 100;
    const num = Math.round(changePct);
    return { txt: `${num}%`, num };
  }
  function priceChg_TodayPlus2_(series, buyStartMs2, d0Px, isHoliday) {
    if (isHoliday) {
      const prev = lastNBefore_(series, buyStartMs2, 3, r => r.close);
      if (prev.length < 3) return { txt: '', num: NaN };
      const [p1, p2, p3] = prev;
      if (!(p1 > 0 && p2 > 0 && p3 > 0)) return { txt: '', num: NaN };
      const c1 = (p1 / p2 - 1) * 100;
      const c2 = (p2 / p3 - 1) * 100;
      const num = Math.round((c1 + c2) / 2);
      return { txt: `${num}%`, num };
    }
    if (!Number.isFinite(d0Px)) return { txt: '', num: NaN };
    const prev = lastNBefore_(series, buyStartMs2, 2, r => r.close);
    if (prev.length < 2) return { txt: '', num: NaN };
    const [p1, p2] = prev;
    if (!(p1 > 0 && p2 > 0)) return { txt: '', num: NaN };
    const c1 = (d0Px / p1 - 1) * 100;
    const c2 = (p1 / p2 - 1) * 100;
    const num = Math.round((c1 + c2) / 2);
    return { txt: `${num}%`, num };
  }
  ...
  function computeTriggerB_(series, buyStartMs2) {
    const hist = series.filter(b => b.ms < buyStartMs2);
    ...
    const atrArr = atr_(hi, lo, cl, ATR_LEN);
  ...
  function predictEntryB_(series, buyStartMs2, ltpPxNow, isHolidayToday2) {
    const t = computeTriggerB_(series, buyStartMs2);
  ...
  for (const r of ltpVals) {
    ...
    const buyDate = predEnt.plannedDate || chooseBuyDate_();
    const buyDateText = predEnt.plannedText || fmtDate(buyDate);

    const stop = Number.isFinite(entry) ? +(entry * (1 - STOP_FLOOR_PCT)).toFixed(2) : NaN;
  ...
  function recalcEntryFromLTP_(predictedEntry, ltpNow, trendScore, hasDemand) {
    if (!Number.isFinite(predictedEntry) || !Number.isFinite(ltpNow)) return predictedEntry;

    const diffPct = ((predictedEntry - ltpNow) / ltpNow) * 100;
    if (Math.abs(diffPct) <= 1.5) return +predictedEntry.toFixed(2);

    let bufferPct;
    if (trendScore >= 3 || hasDemand) bufferPct = 1.0;
    else if (trendScore >= 2) bufferPct = 2.0;
    else bufferPct = 4.0;
  ...
  function passGates_(it) {
    ...
    if (DEF.USE_LIQUIDITY_GUARD && Number.isFinite(it.s.liqMed20) && it.s.liqMed20 < DEF.LIQ_MIN_VAL_MN_MED20) return false;

    if (REQUIRE_VALUE_INCREASE && Number.isFinite(it.valueIncNum) && it.valueIncNum < MIN_VALUE_INC_FILTER_PCT) return false;
    ...
    if (!Number.isFinite(it.entry)) return false;

    const currentLtp = ltpPriceByCode.get(it.code);
    ...
    const ENTRY_TOLERANCE_PCT = 3;
    if (Number.isFinite(currentLtp)) {
      const maxAllowed = it.entry * (1 + ENTRY_TOLERANCE_PCT / 100);
      if (currentLtp > maxAllowed) return false;
    }

    return true;
  }

  const pool = itemsList.filter(passGates_);
  ...
  const listed = pool.filter(x => !pinnedCodes.has(x.code)).map(x => {
    x.action = 'Listed';
    return x;
  });

  listed.sort((a, b) => {
    const sa = Number.isFinite(a.s && a.s.trendScore) ? a.s.trendScore : -Infinity;
    const sb = Number.isFinite(b.s && b.s.trendScore) ? b.s.trendScore : -Infinity;
    if (sb !== sa) return sb - sa;
    const wa = Number.isFinite(a.winPct) ? a.winPct : -Infinity;
    const wb = Number.isFinite(b.winPct) ? b.winPct : -Infinity;
    if (wb !== wa) return wb - wa;
    return String(a.code).localeCompare(String(b.code));
  });

  const merged = [...orderedPurchased, ...orderedWatch, ...listed.slice(0, MAX_CANDIDATES)];
  ...
  function nextPeakReturns_(arr, H) {
    ...
    for (let i = 0; i < arr.length - 1; i++) {
      ...
      const end = Math.min(arr.length - 1, i + H);
      let peak = -Infinity;
      for (let j = i + 1; j <= end; j++) {
        const c = arr[j].close;
        if (Number.isFinite(c) && c > peak) peak = c;
      }
      if (peak > -Infinity) {
        const rt = (peak / e) - 1;
        if (Number.isFinite(rt)) out.push(rt);
      }
    }
    return out;
  }
  ...
  function predictTargetPct_Achievable_(arr, entryPx) {
    ...
    if (ret.length >= MIN_SAMPLES) {
      let tPct = quantile_(ret, 1 - DESIRED_HIT_PROB);
      if (!Number.isFinite(tPct)) tPct = MIN_TARGET_PCT;
      return clamp(tPct, MIN_TARGET_PCT, MAX_TARGET_PCT);
    }
    const hi = arr.map(x => x.high), lo = arr.map(x => x.low), cl = arr.map(x => x.close);
    const a = atr_(hi, lo, cl, ATR_LEN);
    ...
    if (last10.length >= 5) {
      const minC = Math.min.apply(null, last10);
      const maxC = Math.max.apply(null, last10);
      const swingPct = (maxC - minC) / (entryPx || maxC);
  ...
  function computeWinAndHold_(arr, tgtPct) {
    ...
    for (let i = 0; i < arr.length - 1; i++) {
      const e = arr[i].close;
      if (!Number.isFinite(e) || e <= 0) continue;
      const tgt = e * (1 + tgtPct);
      const end = Math.min(arr.length - 1, i + MAX_HOLD_DAYS);
      let hit = false;
      let hold = NaN;
      for (let j = i + 1; j <= end; j++) {
        const c = arr[j].close;
  ...
  const finalRows = merged.map(it => {
    ...
    const exitDate = (() => {
      ...
      const base = isNaN(d) ? chooseBuyDate_() : d;
  ...
  function applyThinBordersToUsedRange_(sh, headersLen) {
    ...
    const lastC = Math.min(sh.getMaxColumns(), headersLen);
  ...
  function writeSummary_(rows, actionList) {
    ...
    if (rows && rows.length) {
      const rule = SpreadsheetApp.newDataValidation().requireValueInList(actionList, true).setAllowInvalid(false).build();
  ...
  function writeLog_(obj) {
    ...
    const lr2 = sh.getLastRow(), lc2 = sh.getLastColumn();
    if (lr2 && lc2) sh.getRange(1, 1, lr2, lc2).clearContent();
