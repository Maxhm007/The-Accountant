...
// Charts layout (Charts sheet: vertical stack)
const CHART_ITEM_WIDTH_PX    = 320;  // a bit wider
const CHART_ITEM_HEIGHT_PX   = 190;  // a bit taller
const CHART_ITEM_HEADER_H_PX = 14;   // smaller header height
...
function generateWeeklyAnalysisPdf_() {
  ...
  /***** ===== Charts sheet: ALL CHARTS ON ONE PAGE, VERTICAL LAYOUT ===== *****/
  ...
  // ---- Vertical layout: charts stacked one below another ----
...
let rTop = 1;
const rGap = 0.003 + Math.ceil(CHART_ITEM_HEIGHT_PX / ROW_HEIGHT_PX);


  function placeHeader(text, rowH) {
    charts.getRange(rowH, 2)
      .setValue(text)
      .setFontSize(10)
      .setFontWeight('bold')
  ...
  // Chart 5: Collections Weekly (top)
  if (c5Rows.length && REPORT_INCLUDE_CHARTS) {
    placeHeader('5. Collections — Weekly (Last 3 Months)', rTop);
  ...
  // Chart 6: Aging Bucket Pie (below chart 5) with fixed colors
  placeHeader('6. Aging Bucket — Share of Outstanding', rTop);
  ...
  const chart6 = charts.newChart().asPieChart()
    ...
    .setOption('width', CHART_ITEM_WIDTH_PX)
    .setOption('height', CHART_ITEM_HEIGHT_PX)
    // COLORS ORDER MUST MATCH ROWS: 0-30, 31-60, 61-90, 90+
    .setOption('colors', [
      '#00b050', // 0-30  -> Green
      '#0070c0', // 31-60 -> Blue
      '#ed7d31', // 61-90 -> Orange
  ...
  // Chart 7: Sales + DSO (below pie chart)
  if (comboRows.length && REPORT_INCLUDE_CHARTS) {
    placeHeader('7. Outstanding by Sales Person (Bars) + DSO (Line)', rTop);
  ...
  // Chart 8: Expected Cash (bottom)
  if (expRows.length && REPORT_INCLUDE_CHARTS) {
    placeHeader(`8. Expected Cashflow (Next ${NEXT_N_DAYS_FOR_EXP_CASH} days)`, rTop);
...
function buildEmailWeeklyTables_(srcSS) {
  ...
  // ----- CSS (NORMAL font, Arial, BLACK borders) -----
...
function fmtBDT_(n) {
  ...
  return Utilities.formatString('%s', (Math.round(n)).toLocaleString('en-US'));
}
